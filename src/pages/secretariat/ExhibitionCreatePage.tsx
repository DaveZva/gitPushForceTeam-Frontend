...
import { exhibitionSchema, ExhibitionFormData } from '../../schemas/exhibitionSchema';
...
export default function ExhibitionCreatePage() {
    ...
    const methods = useForm<ExhibitionFormData>({
    ...
    const onSubmit = async (data: ExhibitionFormData) => {
        ...
        try {
            ...
            navigate('/secretariat/exhibition');
        } catch (error: unknown) {
            if (error instanceof Error) {
                setApiError(error.message || t('alert.createError'));
            } else {
                setApiError(t('alert.createError'));
            }
