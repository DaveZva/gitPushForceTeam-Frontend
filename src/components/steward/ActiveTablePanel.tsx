            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden relative flex flex-col">
                <div className="bg-gray-900 text-white px-6 py-3 flex justify-between items-center relative z-10">
                    <span className="font-bold uppercase text-sm tracking-widest flex items-center gap-2">
                        <span className={`w-2.5 h-2.5 rounded-full ${isPaused ? 'bg-yellow-500' : 'bg-green-500 animate-pulse'}`}></span>
                ...
                {currentCats.length > 0 ? (
                    <div className="p-8 flex-1 flex flex-col justify-between bg-blue-50/20">
                        <div className="flex flex-wrap gap-4 justify-center">
                            {currentCats.map(cat => (
                                <div key={cat.id} className="flex-1 min-w-[240px] max-w-[320px] bg-white p-6 rounded-2xl shadow-md border border-blue-50 flex flex-col items-center text-center transition-all hover:shadow-lg">
                                    <div className="flex items-baseline gap-2 mb-2">
                                        <span className="text-6xl font-black text-gray-900 tracking-[-4px]">#{cat.catalogNumber}</span>
                                        <span className="text-xl text-gray-400 font-bold bg-gray-50 px-2 py-0.5 rounded border border-gray-100">{cat.sex === 'MALE' ? '1.0' : '0.1'}</span>
                                    </div>
                                    <h3 className="text-2xl font-bold text-[#027BFF] mb-6 tracking-[-1px]">{cat.ems}</h3>
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-yellow-500 p-8 min-h-[300px]">
                        <span className="text-3xl font-black mb-2 tracking-[-1px] uppercase animate-pulse italic">{t('steward.pauseJudging')}</span>
                        <span className="text-sm font-bold text-yellow-600/70 tracking-widest uppercase">{t('steward.publicBoardPaused')}</span>
                    </div>
                ) : (
                    <div className="flex-1 flex flex-col items-center justify-center text-gray-400 p-8 min-h-[300px]">
                        <span className="text-xl font-bold mb-1 tracking-[-1px] text-gray-600">{t('steward.tableEmpty')}</span>
                        <span className="text-sm font-medium opacity-60">{t('steward.callNextCat')}</span>
                    </div>
                )}
            </div>
            ...
            {readyCats.length > 0 && (
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {readyCats.map(cat => (
                            <div key={cat.id} className="bg-white border-l-4 border-yellow-400 p-5 rounded-xl shadow-sm border border-gray-100 flex flex-col justify-between transition-all hover:shadow-md">
                                <div className="flex justify-between items-start mb-4">
                                    <div>
                                        <span className="text-4xl font-black text-gray-900 tracking-[-2px]">#{cat.catalogNumber}</span>
                                        <div className="font-bold text-[#027BFF] mt-1 tracking-tight">{cat.ems}</div>
                                    </div>
                                    <span className="text-[10px] font-black text-gray-400 bg-gray-50 px-2.5 py-1 rounded-lg border border-gray-100">{cat.sex === 'MALE' ? '1.0' : '0.1'}</span>
                                </div>
                                <div className="flex gap-2">
                                    <button
